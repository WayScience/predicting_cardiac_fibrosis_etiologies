...
def infer_condition(image_path: Path, plate_folder_pattern: PatternLike) -> str:
    """
    Infer condition from the parent folder of the CARD plate folder.
    ...
    image_path : Path
        Path to image file.
    plate_folder_pattern : PatternLike
        Regex pattern used to identify the plate folder.

    Returns
    -------
    str
        Condition metadata, such as x2 or x3.
    """
    plate_folder_pattern = _compile_pattern(plate_folder_pattern)

    # Iterate through parent folders to find the plate folder and return its parent
    for parent in image_path.parents:
        folder_match = plate_folder_pattern.search(parent.name)
        if folder_match:
...
def parse_image_path(
    ...
    -------
    Dict[str, Any]
        Dictionary containing plate, well, site, condition, and file metadata.
    """
...
def build_loaddata_csv(
    ...
    if duplicate_mask.any():
        ...
        raise ValueError(
            "Found multiple files for the same plate/well/site/condition/channel. "
